...
BEGIN
    ...
    SELECT r.id
    FROM SafeFlow_Update.Budapest r
    WHERE r.nome = startstation;
...
BEGIN
    DELETE FROM SafeFlow_Update.comunicazioni
...
BEGIN
    DELETE FROM SafeFlow_Update.PathInfo
    WHERE Utente = p_codice_fiscale;

    DELETE FROM SafeFlow_Update.Pagamenti
    WHERE codice_fiscale = p_codice_fiscale;
